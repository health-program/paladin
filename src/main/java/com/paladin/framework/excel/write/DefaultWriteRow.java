...
public class DefaultWriteRow extends WriteRow {
	...
	public static WriteRow createWriteRow(Class<?> clazz, EntityField rowField, EntityField... parentField) {
		...
		for (EntityField field : fields) {
			...
			if (writeBean != null) {
				...
				if (field.isCollection()) {
					WriteRow subRow = createWriteRow(field.getCollectionType(), field);
					if (subRow != null) {
						subRows.add(subRow);
					}
				} else if (field.isMap()) {
					...
					WriteRow subRow = createWriteRow(field.getArrayType(), field);
					if (subRow != null) {
						subRows.add(subRow);
					}
				} else {
					...
					WriteRow subRow = createWriteRow(field.getType(), field, parents);
