...
import com.paladin.framework.redis.RedisTemplateFactory;
...
public class ShiroRedisSessionDAO implements SessionDAO {
	...
	String sessionPrefix = "shiro-session"; // session 保存在redis中key的前缀

	@Value("${shiro.session.session-time}")
	int sessionTime = 30; // session 在redis中缓存时间

	@Value("${shiro.session.access-time-update-interval}")
	int accessTimeUpdateInterval = 60 * 1000; // session lastAccessTime 更新间隔
	...
	private String getRedisKey(Serializable sessionId) {
		return sessionPrefix + sessionId.toString();
	}

	private ValueOperations<String, Object> getValueOperations() {
		if (valueOperations == null) {
			synchronized (ShiroRedisSessionDAO.class) {
				if (valueOperations == null) {
					valueOperations = RedisTemplateFactory.getJdkRedisTemplate().opsForValue();
				}
			}
		}
	...
	private void cacheSessioin(Serializable sessionId, Session session) {
		...
		getValueOperations().set(getRedisKey(sessionId), session, sessionTime, TimeUnit.MINUTES);				
	...
	private void uncacheSession(Session session) {
		...
		RedisTemplateFactory.getJdkRedisTemplate().delete(getRedisKey(session.getId()));
	...
	private Session getCacheSession(Serializable sessionId) {
		...
		Session cached = (Session) getValueOperations().get(getRedisKey(sessionId));
	...
	private void updateCacheExpireTime(Serializable sessionId) {
		...
		RedisTemplateFactory.getJdkRedisTemplate().expire(getRedisKey(sessionId), sessionTime, TimeUnit.MINUTES);
				...
				if (controlledSession.isContentChanged) {
					...
					if(accessTimeUpdateInterval <= 0) {
						...
						if (controlledSession.previousLastAccessTime != null) {
							...
							if (now - previous > accessTimeUpdateInterval) {
